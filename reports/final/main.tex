...
We binarize 15+ neighbourhood features against their medians and apply the Apriori algorithm (Agrawal \& Srikant, 1994; mlxtend~0.23.4) with minimum support$=0.15$, minimum confidence$=0.5$, minimum lift$=1.0$ and maximum itemset length$=3$. Features include high density, low stop density, high transit commute share, low income, high seniors share, high immigrant share, underserved status, and poverty indicators from both Low Income Measure After Tax (LIM-AT) and Market Basket Measure (MBM).

\subsection{Clustering}

K-means clustering (MacQueen, 1967; $k=4$, selected via elbow method with silhouette validation) groups routes by normalized hourly departure profiles, capturing when each route operates throughout the day. We validate with Calinski-Harabasz and Davies-Bouldin indices and compare against DBSCAN to assess cluster stability. Ward hierarchical clustering validates the K-means solution: the Adjusted Rand Index between K-means and hierarchical assignments confirms agreement.
...
An interactive 8-tab Streamlit dashboard was developed to support exploration and interpretation of the analytical results in a more accessible way. The Overview tab presents summary metrics including route counts, stop counts, and changes in ridership. The Map tab renders the full transit network using PyDeck with PTN tiers colour-coded to clearly show differences in service structure. The Coverage tab examines stop density and jobs accessibility to identify underserved neighbourhoods, while the Network tab ranks key hubs using betweenness and PageRank centrality and visualizes Louvain communities. The Frequency tab summarizes scheduled service patterns including headway comparisons across PTN tiers, and the Live tab connects to the Winnipeg Transit API to display real-time vehicle positions and service alerts.

Particular attention is given to the Equity Deep Dive and Densification Alignment tabs. The Equity Deep Dive tab presents the income quintile analysis (Table~\ref{tab:quintile}), as shown in Figure~\ref{fig:dashboard}, and allows for neighbourhood-level comparison of transit access and stop density. The results show a non-linear relationship between income and access, where the lowest-income quintile has the highest median access score at 52.63, followed by Q2 at 38.28, Q3 at 28.83, Q5 at 21.78, and Q4 at 20.63. This pattern reflects the concentration of transit infrastructure in inner-city areas and highlights that higher proximity does not necessarily correspond to better service quality.

The Densification Alignment tab overlays current stop locations with OurWinnipeg 2045 planning zones, including mixed-use corridors, regional centres, and redevelopment areas. This view highlights where existing transit service aligns with planned growth and where gaps remain. In combination with the equity analysis, this makes it possible to identify areas where future densification is not currently supported by sufficient transit infrastructure.

To ensure consistency, the dashboard reads from a curated serving database rather than recomputing results dynamically. This guarantees that the interactive outputs reflect the same validated results presented in this report.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.85\textwidth]{equity_dashboard}\\[6pt]
\includegraphics[width=0.85\textwidth]{densification_dashboard}
\caption{Dashboard views. Top: Equity Deep Dive showing income quintile accessibility. Bottom: Densification Alignment showing transit coverage relative to planned growth zones.}
\label{fig:dashboard}
\end{figure}
...
Transit accessibility scores (Figure~\ref{fig:equity_quintile}) reveal an inverse income pattern: Q1 neighbourhoods have the \emph{highest} median access score (52.63), while Q5 has the lowest (21.78). The pattern is broadly inverse: Q1 (52.63) $>$ Q2 (38.28) $>$ Q3 (28.83), though Q4 (20.63) dips below Q5 (21.78), reflecting suburban pockets with marginally better feeder coverage. This reflects Winnipeg's urban geography: the poorest neighbourhoods are inner-city (Lord Selkirk Park, Spence, West End) with high stop density, while the wealthiest (Tuxedo, Wellington Crescent) are low-density suburbs far from transit spines.

On paper, this looks like an equity success: lower-income neighbourhoods are closer to more stops and therefore receive higher proximity-based accessibility scores. But this is where Lucas' (2012) ``double penalty'' becomes important. Transport disadvantage is not only about whether transit exists nearby, but also about whether vulnerable residents can use it to reach jobs, services and daily destinations reliably. In Winnipeg's case, Q1 neighbourhoods may have the highest measured proximity, yet they also face the greatest dependence on transit, the highest exposure to transfer friction, and the fewest practical alternatives when service fails. The first penalty is dependence on a system that is already fragile; the second is reduced access to wider opportunities when reliability, wait times and transfer quality are poor.

The PTN therefore produces a distinction between \emph{paper equity} and \emph{lived equity}. Proximity improved most for the lowest-income quintile, but proximity alone does not capture pass-ups, missed transfers, winter walking conditions or the burden of converting direct trips into feeder--spine--feeder chains. Q1's higher access score should not be read as evidence that low-income riders are better served overall. Rather, it shows that the redesign concentrated service geometry in the inner city while leaving unresolved the service-quality barriers that shape everyday travel experience. The policy implication is that equity cannot be assessed only by counting nearby stops; it must also account for the reliability and usability of the journeys those stops enable.
...
\caption{Median and mean transit access score by income quintile. Q1 (lowest income, inner-city) shows the highest scores; Q4 the lowest. The Q1$\to$Q5 median gap is $-58.9$\%: lower-income neighbourhoods are closer to more stops, but proximity does not equal service quality.}
...
The poverty-transit correlation confirms the same pattern observed in the quintile analysis: lower neighbourhood income is associated with \emph{higher} measured transit access, not lower. In other words, Winnipeg's poorest areas are not transit deserts in a purely geometric sense. They are often close to stops and major corridors, especially in the inner city. But Allen and Farber's (2020) accessibility framework helps explain why this does not automatically translate into equitable mobility outcomes. Accessibility is shaped not just by spatial proximity, but by whether the network actually connects people to jobs and services with acceptable travel times, transfer burdens and reliability. A stop within walking distance is not the same thing as a low-friction trip.

The coverage change choropleth (Figure~\ref{fig:coverage_change}) shows that stop-density change was uneven across the city. Several peripheral and lower-density neighbourhoods experienced little improvement or outright decline, while gains were concentrated in selected corridors and growth areas. The underserved-neighbourhood ranking (Figure~\ref{fig:underserved_ranks}) reinforces this pattern: the weakest-served neighbourhoods are concentrated in low-density peripheral zones, and 16 neighbourhoods have zero stops entirely. This means the PTN did not simply redistribute service from rich to poor or from suburbs to the core; it produced a patchwork geography in which some outer areas lost coverage while many lower-income inner-city areas retained proximity but remained vulnerable to poor trip quality.

This matters because equity weighting materially changes who appears most urgent for intervention. Our prescriptive counterfactual reprioritizes 209 of 237 neighbourhoods (88.2\%), showing that the default proximity-based ranking substantially understates vulnerability. The largest gainers under equity weighting are Alpine Place (+44 ranks), Valhalla (+37) and Pembina Strip (+36), all of which move upward once transit dependence and social vulnerability are incorporated. The broader lesson is that densification and coverage must be evaluated together with poverty and dependence: a network can look spatially rational on a map while still reproducing transport disadvantage for the riders who rely on it most.
...
\caption{Median household income vs.\ transit access score across all 237 neighbourhoods ($R^2 = 0.218$, regression line shown). Lower income correlates with higher proximity-based access scores, the inverse of what standard equity models predict, but proximity does not capture service quality.}
...
\caption{Prescriptive counterfactual: change in priority rank under equity weighting. Green bars to the right show neighbourhoods that gained rank because vulnerability indices (low income, transit dependence, seniors share) were incorporated into the objective; red bars to the left show neighbourhoods that lost rank. Top gainers include Alpine Place ($+44$), Valhalla ($+37$) and Pembina Strip ($+36$); top losers include affluent suburbs like Wellington Crescent and Tuxedo. 209 of 237 neighbourhoods (88.2\%) reprioritized.}
...
Edmonton's April 2021 redesign achieved over 15\% ridership growth by 2024 (Edmonton Transit Service, 2024) through iterative post-launch adjustments, historically higher per-capita spending (CCPA 2018 documented \$335 vs.\ \$200 based on 2015 Canadian Urban Transit Association data), and existing light rail transit (LRT) infrastructure. Winnipeg replaced every route overnight on June 29, 2025 with no parallel service, no transition, no way to learn the new map gradually. Riders woke up and their bus was gone. Edmonton iterated. Winnipeg imposed.
...
\begin{itemize}[nosep]
\item Coverage and equity analysis conceptualisation, including neighbourhood density, underserved-area identification, and income-based accessibility comparison
\item Produced the coverage and equity analysis results, including income-quintile accessibility, poverty overlay, coverage change, underserved neighbourhood ranking, and equity-weighted reranking (\texttt{1.3-sudipta-pr2-coverage.ipynb} and \texttt{1.4-sudipta-final-equity.ipynb})
\item Generated and validated Figures 6--10 outputs used for income quintile analysis, poverty/access correlation, coverage change mapping, underserved neighbourhood ranking, and equity-weighted reranking
\item Wrote Sections 5.7 (Equity by Income Quintile) and 5.8 (Poverty Overlay and Coverage Densification)
\item Interpreted the coverage/equity figures.
...
Cathy Li & & \\
Sudipta Sarker & SS & April 5, 2026 \\
Stephenie Michael & SCM & April 6, 2026 \\
\bottomrule
\end{tabular}
\end{table}
